...
export default function ShopItem({ item, pickaxeContract }: Props) {
    const { data: claimCondition } = useActiveClaimCondition(pickaxeContract, item.metadata.id)
    ...
    return (
        <div className={styles.nftBox} key={item.metadata.id.toString()}>
            ...
            <p>
                Price:{" "}
                <b>{claimCondition && ethers.utils.formatUnits(claimCondition?.price)} GEM</b>
            </p>
