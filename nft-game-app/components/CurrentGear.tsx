...
export default function CuurentGear({ miningContract, characterContract, pickaxeContract }: Props) {
    ...
    useEffect(() => {
        ;(async () => {
            ...
            // Now we have the tokenId of equipped pickaxe, If there is one , fetch the metadata for it
            if (p.isData) {
                const pickaxeMetadata = await pickaxeContract.get(p.value)
                setPickaxe(pickaxeMetadata)
            }
        })()
    }, [address, miningContract, pickaxeContract]) // If this 3 changes, then repeat the above things in useEffect
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            >
                ...
                <div style={{ outline: "1px solid grey", borderRadius: 16 }}>
                    {playerNft && <ThirdwebNftMedia metadata={playerNft?.metadata} height={"64"} />}
                </div>
