...
contract Mining is ReentrancyGuard, ERC1155Holder {
    ...
    // Constructor function to set the rewards token and NFT collection addresses
    ...
    // Mapping of addresses to pickaxe is not set until they stake one.
    // In this example, the tokenId of pickaxe is the multiplier for the reward.
    mapping(address => MapValue) public playerPickaxe;

    // Mapping of player address until last time they staked/withdraw/claimed their rewards.
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        //Upadate the playerLastUpdate mapping
        playerLastUpdate[msg.sender].isData = true;
        playerLastUpdate[msg.sender].value = block.timestamp;
    }

    function witdraw() external nonReentrant {
        // Ensure the player has a pickaxe
        require(playerPickaxe[msg.sender].isData, "You do not have a pickaxe withdraw");
